...
public class ProblemaController : Controller
{
    ...
    public ProblemaController(Judge0Service judge0Service, ApplicationDbContext context)
    {
        ...
        List<(string input, string output)> casos = new()
    {
        ...
        foreach (var caso in casos)
        {
            try
            {
                var resultadoJson = await _judge0Service.EnviarCodigoAsync(codigo, caso.input, lenguaje);
